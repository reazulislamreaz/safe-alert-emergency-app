      ...
      <div className="flex items-center gap-1 sm:gap-4 shrink-0">
        <div className="relative">
          ...
          >
            ...
            <span className={`absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-red-500 border border-white ${unreadCount > 0 ? '' : 'hidden'}`} />
          </button>
              ...
              <div className="mt-2 space-y-1.5 text-xs text-gray-600">
                {notifications.map((item) => (
                  <div
                    ...
                    className={
                      item.read
                        ? 'p-2 rounded-xl bg-gray-50 text-gray-700'
                        : 'p-2 rounded-xl bg-red-50 border border-red-100 text-red-700'
                    }
                  >
                    <p className="font-bold text-[11px]">{item.title}</p>
                    <p className={item.read ? 'text-[10px] text-gray-500' : 'text-[10px] text-red-600'}>
                      {item.body}
                    </p>
                  </div>
                ))}
